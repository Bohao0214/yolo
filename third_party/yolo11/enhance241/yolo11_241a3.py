...
class SPDConvDownsample(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if recorder is not None:
            if step == 0:
                baseline = None
                baseline_mod = _get_module_baseline(self)
                if baseline_mod is not None:
                    try:
                        with torch.no_grad():
                            baseline = baseline_mod(x_in.detach())
                    except Exception as exc:
                        recorder.add_note(f"a3_baseline_forward_failed:{exc}")
                recorder.record_module_compare(f"{prefix}.p3_down", patched=x, baseline=baseline, input_tensor=x_in)
                if x.requires_grad:
                    try:
                        x.register_hook(lambda g, n=f"{prefix}.p3_down": recorder.record_output_grad(n, g))
                    except Exception:
                        pass
            elif step == 1:
...
def apply(model: Any, cfg: Any) -> Any:
    ...
    if recorder is not None:
        try:
            baseline_mod = old
            baseline_mod.eval()
            for p in baseline_mod.parameters():
                p.requires_grad_(False)
            _bind_module_debug(fuse, recorder, prefix=f"a3.idx{ds_idx}", baseline=baseline_mod)
        except Exception as exc:
            recorder.add_note(f"a3_baseline_attach_failed:{exc}")
