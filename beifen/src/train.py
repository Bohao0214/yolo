...
from third_party.yolo11.hard_mining import apply_hard_mining
from third_party.yolo11.patches import apply_focal_loss, get_extra_train_args
from third_party.yolo11.anti_fp import apply_fp_enhancements
from eval import (
    compute_ap,
    compute_auc,
...
def main() -> None:
    ...
    apply_fp_enhancements(model, cfg)
    run_data_yaml = apply_hard_mining(model, run_data_yaml, exp_dir, cfg)
    extra_train_args = get_extra_train_args(cfg)
    ...
    def eval_split(tag: str, source: Path) -> None:
        ...
        try:
            ...
            pass
        try:
            labels, scores = compute_image_scores(model, source, metric_conf, eval_batch, eval_device, nms_iou)
            ...
            try:
                ...
                pass
            gc.collect()
            labels, scores = compute_image_scores(model, source, metric_conf, 1, "cpu", nms_iou)
        if labels.size == 0:
            return
            ...
            for iou_thr in fix_list:
                try:
                    labels_i, scores_i = compute_image_scores_iou(
                        model, source, metric_conf, float(iou_thr), eval_batch, eval_device, nms_iou
                    )
                except RuntimeError as exc:
                    if "out of memory" not in str(exc).lower():
                        raise
                    ...
                    gc.collect()
                    labels_i, scores_i = compute_image_scores_iou(
                        model, source, metric_conf, float(iou_thr), 1, "cpu", nms_iou
                    )
                if labels_i.size == 0:
                    continue
