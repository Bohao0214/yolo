...
# Path: third_party/yolo11/enhance241/yolo11_241d11.py
# Purpose: enhance241 d11 (residual-safe cls score calibration with temperature + bias).
...
ENHANCE241_AUDIT_KEYS = ["enhance241_d11"]  # enhance241-audit
...
class D11ClsScoreCalib(torch.nn.Module):
    ...
    def __init__(
        ...
        self.enhance241_d11_alpha_raw = torch.nn.Parameter(alpha_raw)  # enhance241-audit

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        recorder = _get_module_recorder(self)
        prefix = str(getattr(self, "_enhance241_prefix", "d11"))
...
def apply(model: Any, cfg: Any) -> Any:
    ...
    if seq is None:
        raise RuntimeError("enhance241.d11 requires YOLO/DetectionModel-like object with .model sequence.")
    ...
    if cls_heads is None or not isinstance(cls_heads, torch.nn.ModuleList) or len(cls_heads) < 1:
        raise RuntimeError("enhance241.d11 expects Detect.cv3 ModuleList with at least one head.")

    apply_to = _parse_apply_to(_deep_get(cfg, "enhance241", "d11_apply_to", default=["P3", "P2_new"]))
    target_heads = _target_head_indices(detect, apply_to)
    if not target_heads:
        raise RuntimeError(f"enhance241.d11 apply_to={apply_to} matched no valid detect heads.")

    temp_init = _safe_float(_deep_get(cfg, "enhance241", "d11_temp_init", default=1.0), 1.0)
    ...
    existing = 0
    recorder = get_check_recorder("d11", cfg)
    for head_idx in target_heads:
        old_cls = cls_heads[head_idx]
        if isinstance(old_cls, D11ClsScoreCalib):
            existing += 1
            continue

        wrapped = D11ClsScoreCalib(
            old_cls,
            temp_init=temp_init,
            t_min=t_min,
            t_max=t_max,
        ...
        if recorder is not None:
            try:
                _bind_module_debug(wrapped, recorder, prefix=f"d11.detect{detect_idx}.head{head_idx}")
                recorder.register_module_params(wrapped, f"d11.detect{detect_idx}.head{head_idx}")
            except Exception as exc:
                recorder.add_note(f"d11_attach_debug_failed:{head_idx}:{exc}")
        ...
        if isinstance(one2one_cv3, torch.nn.ModuleList) and head_idx < len(one2one_cv3):
            ...
            if not isinstance(old_o2o, D11ClsScoreCalib):
                wrapped_o2o = D11ClsScoreCalib(
                    old_o2o,
                    temp_init=temp_init,
                    t_min=t_min,
                    t_max=t_max,
    ...
    if recorder is not None:
        ...
        try:
            ...
            recorder.add_note(f"d11_detect_hook_failed:{exc}")
        try:
            recorder.maybe_run_val_separability(yolo_obj, cfg)
        except Exception as exc:
            recorder.add_note(f"d11_val_check_failed:{exc}")
